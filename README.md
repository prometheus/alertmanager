...
`amtool` allows a configuration file to specify some options for convenience. The default configuration file paths are `$HOME/.config/amtool/config.yml` or `/etc/amtool/config.yml` or `C:\Users\%USER%\AppData\Roaming\amtool\config.yml` and `C:\Programdata\amtool\config.yml`.
