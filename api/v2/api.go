...
func CheckSilenceMatchesFilterLabels(s *silencepb.Silence, matchers []*matcher.Matcher) bool {
	for _, mi := range matchers {
		found := false
		for _, mj := range s.Matchers {
			if mi.Name == mj.Name &&
				(mi.Type == matcher.MatchEqual && mj.Type == silencepb.Matcher_EQUAL ||
					mi.Type == matcher.MatchRegexp && mj.Type == silencepb.Matcher_REGEXP ||
					mi.Type == matcher.MatchNotEqual && mj.Type == silencepb.Matcher_NOT_EQUAL ||
					mi.Type == matcher.MatchNotRegexp && mj.Type == silencepb.Matcher_NOT_REGEXP) &&
				mi.Value == mj.Pattern {
				found = true
				break
			}
