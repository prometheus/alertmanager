...
func configureTemplateRenderCmd(cc *kingpin.CmdClause) {
	...
	renderCmd.Flag("template.delimiter.left", "The left action delimiter string (default {{)").EnumVar(&c.templateDelimLeft)
	renderCmd.Flag("template.delimiter.right", "The right action delimiter string (default }})").EnumVar(&c.templateDelimLeft)
...
func (c *templateRenderCmd) render(ctx context.Context, _ *kingpin.ParseContext) error {
	...
	err = tmpl.Delims(c.templateDelimLeft, c.templateDelimRight).ParseGlobs(c.templateFilesGlobs...)
	if err != nil {
		return err
	}
