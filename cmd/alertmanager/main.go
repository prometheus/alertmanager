...
func run() int {
	...
	var (
		...
		metricCollectInterval = kingpin.Flag("metric-collect-interval", "Interval between metric collection").Default("15s").Duration()
	...
	wg.Add(1)
	go func() {
		silences.CountSilences(*metricCollectInterval, stopc)
		wg.Done()
	}()
