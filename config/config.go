...
func (c *Config) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	for _, rcv := range c.Receivers {
		...
		for _, slc := range rcv.SyslogConfigs {
			// These three have defaults, so if the global is unset the effect will be the same.
			if slc.Tag == "" {
				slc.Tag = c.Global.SyslogTag
			}
			if slc.Daemon.Network == "" {
				slc.Daemon.Network = c.Global.SyslogDaemonNetwork
			}
			if slc.Daemon.Hostname == "" {
				slc.Daemon.Hostname = c.Global.SyslogDaemonHostname
			}

			// We have to evaluate if a daemon connection is even being used before
			// checking if the port is empty.
			// Although this check might result in an empty port getting past here,
			// it will get caught later on.
			if slc.Daemon.Port == 0 && slc.Daemon.Network != "" || slc.Daemon.Hostname != "" {
				if c.Global.SyslogDaemonPort == 0 {
					return fmt.Errorf("no syslog daemon port set")
				}
			}
		}
...
type GlobalConfig struct {
	...
	SyslogDaemonNetwork  string     `yaml:"syslog_daemon_network,omitempty" json:"syslog_daemon_network,omitempty"`
	SyslogDaemonHostname string     `yaml:"syslog_daemon_hostname,omitempty" json:"syslog_daemon_hostname,omitempty"`
	SyslogDaemonPort     int        `yaml:"syslog_daemon_port,omitempty" json:"syslog_daemon_port,omitempty"`
}
