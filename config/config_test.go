...
func TestRocketchatDefaultTokenFile(t *testing.T) {
	...
	defaultToken := conf.Global.RocketchatTokenFile
	overrideToken := "/override_file"
	if defaultToken != conf.Receivers[0].RocketchatConfigs[0].TokenFile {
		t.Fatalf("Invalid Rocketchat key_file: %s\nExpected: %s", conf.Receivers[0].RocketchatConfigs[0].TokenFile, defaultToken)
	}
	if overrideToken != conf.Receivers[1].RocketchatConfigs[0].TokenFile {
...
func TestRocketchatDefaultIDTokenFile(t *testing.T) {
	...
	defaultTokenID := conf.Global.RocketchatTokenIDFile
	overrideTokenID := "/override_file"
	if defaultTokenID != conf.Receivers[0].RocketchatConfigs[0].TokenIDFile {
		t.Fatalf("Invalid Rocketchat key_file: %s\nExpected: %s", conf.Receivers[0].RocketchatConfigs[0].TokenIDFile, defaultTokenID)
	}
	if overrideTokenID != conf.Receivers[1].RocketchatConfigs[0].TokenIDFile {
		t.Errorf("Invalid Rocketchat key_file: %s\nExpected: %s", conf.Receivers[0].RocketchatConfigs[0].TokenIDFile, overrideTokenID)
	}
}
