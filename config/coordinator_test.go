...
func TestCoordinatorRegistersMetrics(t *testing.T) {
	fr := fakeRegisterer{}
	NewCoordinator(NewFileLoader("testdata/conf.good.yml"), &fr, promslog.NewNopLogger())
...
func TestCoordinatorNotifiesSubscribers(t *testing.T) {
	...
	c := NewCoordinator(NewFileLoader("testdata/conf.good.yml"), prometheus.NewRegistry(), promslog.NewNopLogger())
