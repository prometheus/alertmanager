...
route:
  ...
  - match:
	...
	alerts, err := mem.NewAlerts(context.Background(), marker, time.Hour, 0, nil, logger, reg)
	if err != nil {
		t.Fatal(err)
	}
	defer alerts.Close()
...
func TestDispatcher_DoMaintenance(t *testing.T) {
	...
	alerts, err := mem.NewAlerts(context.Background(), marker, time.Minute, 0, nil, promslog.NewNopLogger(), r)
	if err != nil {
		t.Fatal(err)
	}
