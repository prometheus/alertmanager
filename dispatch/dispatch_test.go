...
func TestAggrGroup(t *testing.T) {
	...
	ag := newAggrGroup(context.Background(), lset, route, nil, types.NewMarker(prometheus.NewRegistry()), eventlog.NopRecorder(), promslog.NewNopLogger())
	go ag.run(ntfy)
...
route:
  ...
  - match:
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, testMaintenanceInterval, nil, logger, eventlog.NopRecorder(), NewDispatcherMetrics(false, reg))
...
route:
  ...
  - match:
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, testMaintenanceInterval, lim, logger, eventlog.NopRecorder(), m)
	go dispatcher.Run(time.Now())
	defer dispatcher.Stop()
...
func TestDispatcherRace(t *testing.T) {
	...
	timeout := func(d time.Duration) time.Duration { return time.Duration(0) }
	route := &Route{}
	dispatcher := NewDispatcher(alerts, route, nil, marker, timeout, testMaintenanceInterval, nil, logger, eventlog.NopRecorder(), NewDispatcherMetrics(false, reg))
	go dispatcher.Run(time.Now())
	dispatcher.Stop()
}
...
func TestDispatcher_DoMaintenance(t *testing.T) {
	...
	alerts, err := mem.NewAlerts(context.Background(), marker, time.Minute, 0, nil, promslog.NewNopLogger(), eventlog.NopRecorder(), r, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	ctx := context.Background()
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, testMaintenanceInterval, nil, promslog.NewNopLogger(), eventlog.NopRecorder(), NewDispatcherMetrics(false, r))
	...
	aggrGroup1 := newAggrGroup(ctx, labels, route, timeout, types.NewMarker(prometheus.NewRegistry()), eventlog.NopRecorder(), promslog.NewNopLogger())
...
func TestDispatcher_DeleteResolvedAlertsFromMarker(t *testing.T) {
	t.Run("successful flush deletes markers for resolved alerts", func(t *testing.T) {
		...
		ag := newAggrGroup(ctx, labels, route, timeout, marker, eventlog.NopRecorder(), logger)
