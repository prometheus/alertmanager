...
| join             | sep string, any            | Similar to [strings.Join](http://golang.org/pkg/strings/#Join) for string slices, but accepts a slice or array of type `any`: non-string elements are converted to their string representation (for example via `fmt.Sprint`) before concatenation. This conversion does not return an error; invalid input types can still error per implementation. The separator string `sep` is placed between elements in the resulting string. (note: argument order inverted for easier pipelining in templates.) |
