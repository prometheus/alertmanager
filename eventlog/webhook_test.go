...
func mustParseURL(t *testing.T, raw string) *commoncfg.URL {
	t.Helper()
	u, err := url.Parse(raw)
...
func TestWebhookOutput_WriteEvent(t *testing.T) {
	var received [][]byte
	var mu sync.Mutex
	...
	cfg := config.EventLogOutput{
		Type: "webhook",
		URL:  u,
	}
	wo, err := NewWebhookOutput(cfg, slog.Default())
	require.NoError(t, err)
	defer wo.Close()

	require.Equal(t, "webhook:"+srv.URL, wo.Name())

	require.NoError(t, wo.WriteEvent([]byte(`{"test":"data"}`)))
	...
	mu.Lock()
	require.Equal(t, `{"test":"data"}`, string(received[0]))
	mu.Unlock()
}
...
func TestWebhookOutput_MultipleWorkers(t *testing.T) {
	...
	const n = 50
	for range n {
		require.NoError(t, wo.WriteEvent([]byte(`{}`)))
	}
...
func TestWebhookOutput_RetryOnFailure(t *testing.T) {
	...
	require.NoError(t, wo.WriteEvent([]byte(`{"retry":"test"}`)))
...
func TestWebhookOutput_DropsAfterMaxRetries(t *testing.T) {
	...
	require.NoError(t, wo.WriteEvent([]byte(`{"drop":"test"}`)))
