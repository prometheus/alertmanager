...
func TestBucketGC(t *testing.T) {
	t.Run("GC on empty bucket should not panic", func(t *testing.T) {
		bucket := NewBucket[model.Fingerprint](5)

		// Should not panic when bucket is empty and return true
		require.NotPanics(t, func() {
			empty := bucket.GC()
			require.True(t, empty, "GC on empty bucket should return true")
		}, "GC on empty bucket should not panic")
	})

	t.Run("GC removes all items when latest is expired", func(t *testing.T) {
		...
		require.Len(t, bucket.items, 3, "Bucket should have 3 items before GC")
		require.Len(t, bucket.index, 3, "Bucket index should have 3 items before GC")

		// GC should clear all items and return true
		empty := bucket.GC()

		require.True(t, empty, "GC should return true when all items are expired")
		require.Empty(t, bucket.items, "Bucket should be empty after GC")
		require.Empty(t, bucket.index, "Bucket index should be empty after GC")
	})

	t.Run("GC does not remove items when latest is not expired", func(t *testing.T) {
		bucket := NewBucket[model.Fingerprint](3)
		...
		// GC should not remove anything since the latest item (alert3) is not expired
		empty := bucket.GC()

		require.False(t, empty, "GC should return false when latest item is not expired")
		require.Len(t, bucket.items, 3, "Bucket should still have 3 items after GC when latest is not expired")
		require.Len(t, bucket.index, 3, "Bucket index should still have 3 items after GC")
	})

	t.Run("GC is thread-safe with concurrent Upsert", func(t *testing.T) {
		bucket := NewBucket[model.Fingerprint](100)

		// Pre-populate bucket with expired items
		for i := range 50 {
			alert := model.Alert{
		...
		// Run Upsert concurrently
		go func() {
			for i := range 100 {
				alert := model.Alert{
					Labels: model.LabelSet{"alertname": model.LabelValue(fmt.Sprintf("NewAlert%d", i))},
					EndsAt: time.Now().Add(1 * time.Hour),
				}
				bucket.Upsert(alert.Fingerprint(), alert.EndsAt)
			}
