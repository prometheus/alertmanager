...
func (s *testInhibitorScenario) test(i int, t *testing.T) {
	// Set inhibit rules in constructor.
	inhibitor := NewInhibitor(s.rules)
	labelSets := append(s.inhibited, s.uninhibited...)
	filtered := inhibitor.Filter(labelSets)
	labelSetsMustBeEqual(i, t, s.uninhibited, filtered)

	// Set the inhibit rules to an empty list.
	inhibitor.SetInhibitRules(nil)
	filtered = inhibitor.Filter(labelSets)
	labelSetsMustBeEqual(i, t, labelSets, filtered)

	// Re-add inhibit rules through SetInhibitRules().
