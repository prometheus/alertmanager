...
func TestFallbackMatcherParser(t *testing.T) {
	tests := []struct {
		...
		// the fallback parser returns the result from pkg/matcher.
		name:     "input causes disagreement",
		input:    "foo=\"\\xf0\\x9f\\x99\\x82\"",
		expected: mustNewMatcher(t, matcher.MatchEqual, "foo", "\\xf0\\x9f\\x99\\x82"),
	}}
...
func TestFallbackMatchersParser(t *testing.T) {
	tests := []struct {
		...
		expected: matcher.Matchers{
			...
			mustNewMatcher(t, matcher.MatchEqual, "bar", "baz"),
		},
	}, {
		name:  "input is accepted in neither",
		input: "{foo!bar}",
		...
		input: "{foo=\"\\xf0\\x9f\\x99\\x82\"}",
		expected: matcher.Matchers{
			mustNewMatcher(t, matcher.MatchEqual, "foo", "\\xf0\\x9f\\x99\\x82"),
		},
	}}
