...
var (
	...
	typeMap = map[string]matchers.MatchType{
		"=":  matchers.MatchEqual,
		"!=": matchers.MatchNotEqual,
		"=~": matchers.MatchRegexp,
		"!~": matchers.MatchNotRegexp,
	}
)
...
func ParseMatchers(s string) (matchers.Matchers, error) {
	matchers := make(matchers.Matchers, 0)
...
func ParseMatcher(s string) (_ *matchers.Matcher, err error) {
	...
	return matchers.NewMatcher(typeMap[ms[2]], ms[1], value.String())
}
