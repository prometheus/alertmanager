...
func (s state) merge(e *pb.MeshEntry, now time.Time, remote bool) bool {
...
func (l *Log) Merge(b []byte) error {
	...
	for _, e := range st {
		if merged := l.st.merge(e, now, true); merged && !cluster.OversizedMessage(b) {
