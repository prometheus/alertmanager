...
func (n *Email) Notify(ctx context.Context, as ...*types.Alert) (context.Context, bool, error) {
	var (
		c       *smtp.Client
		conn    net.Conn
		err     error
	...
	if n.conf.Smarthost.Port == "465" {
		...
		if err != nil {
			return ctx, false, fmt.Errorf("parse TLS configuration: %w", err)
		}
		...
		if err != nil {
			return ctx, true, fmt.Errorf("establish TLS connection to server: %w", err)
		}
	} else {
		var (
			d   = net.Dialer{}
		...
		if err != nil {
			return ctx, true, fmt.Errorf("establish connection to server: %w", err)
		}
	}
	c, err = smtp.NewClient(conn, n.conf.Smarthost.Host)
	if err != nil {
		conn.Close()
		return ctx, true, fmt.Errorf("create SMTP client: %w", err)
	}
	defer func() {
	...
	if n.conf.Hello != "" {
		...
		if err != nil {
			return ctx, true, fmt.Errorf("send EHLO command: %w", err)
		}
	}
	...
	if *n.conf.RequireTLS {
		if ok, _ := c.Extension("STARTTLS"); !ok {
			return ctx, true, fmt.Errorf("'require_tls' is true (default) but %q does not advertise the STARTTLS extension", n.conf.Smarthost)
		}
		...
		if err := c.StartTLS(tlsConf); err != nil {
			return ctx, true, fmt.Errorf("send STARTTLS command: %w", err)
		}
	}
	...
	if ok, mech := c.Extension("AUTH"); ok {
		...
		if err != nil {
			return ctx, true, fmt.Errorf("find auth mechanism: %w", err)
		}
		if auth != nil {
			if err := c.Auth(auth); err != nil {
				return ctx, true, fmt.Errorf("%T auth: %w", auth, err)
			}
		}
	}
	...
	if tmplErr != nil {
		return ctx, false, fmt.Errorf("execute 'from' template: %w", tmplErr)
	}
	to := tmpl(n.conf.To)
	if tmplErr != nil {
		return ctx, false, fmt.Errorf("execute 'to' template: %w", tmplErr)
	}

	addrs, err := mail.ParseAddressList(from)
	if err != nil {
		return ctx, false, fmt.Errorf("parse 'from' addresses: %w", err)
	}
	if len(addrs) != 1 {
		return ctx, false, fmt.Errorf("must be exactly one 'from' address (got: %d)", len(addrs))
	}
	if err = c.Mail(addrs[0].Address); err != nil {
		return ctx, true, fmt.Errorf("send MAIL command: %w", err)
	}
	addrs, err = mail.ParseAddressList(to)
	if err != nil {
		return ctx, false, fmt.Errorf("parse 'to' addresses: %w", err)
	}
	for _, addr := range addrs {
		if err = c.Rcpt(addr.Address); err != nil {
			return ctx, true, fmt.Errorf("send RCPT command: %w", err)
		}
	}
	...
	message, err := c.Data()
	if err != nil {
		return ctx, true, fmt.Errorf("send DATA command: %w", err)
	}
	defer message.Close()
	...
	for header, t := range n.conf.Headers {
		...
		if err != nil {
			return ctx, false, fmt.Errorf("execute %q header template: %w", header, err)
		}
	...
	if err != nil {
		return ctx, false, fmt.Errorf("write headers: %w", err)
	}
	...
	if len(n.conf.Text) > 0 {
		...
		if err != nil {
			return ctx, false, fmt.Errorf("create part for text template: %w", err)
		}
		body, err := n.tmpl.ExecuteTextString(n.conf.Text, data)
		if err != nil {
			return ctx, false, fmt.Errorf("execute text template: %w", err)
		}
		qw := quotedprintable.NewWriter(w)
		_, err = qw.Write([]byte(body))
		if err != nil {
			return ctx, true, fmt.Errorf("write text part: %w", err)
		}
		err = qw.Close()
		if err != nil {
			return ctx, true, fmt.Errorf("close text part: %w", err)
		}
	}
	...
	if len(n.conf.HTML) > 0 {
		...
		if err != nil {
			return ctx, false, fmt.Errorf("create part for html template: %w", err)
		}
		body, err := n.tmpl.ExecuteHTMLString(n.conf.HTML, data)
		if err != nil {
			return ctx, false, fmt.Errorf("execute html template: %w", err)
		}
		qw := quotedprintable.NewWriter(w)
		_, err = qw.Write([]byte(body))
		if err != nil {
			return ctx, true, fmt.Errorf("write HTML part: %w", err)
		}
		err = qw.Close()
		if err != nil {
			return ctx, true, fmt.Errorf("close HTML part: %w", err)
		}
	}

	err = multipartWriter.Close()
	if err != nil {
		return ctx, false, fmt.Errorf("close multipartWriter: %w", err)
	}

	_, err = message.Write(multipartBuffer.Bytes())
	if err != nil {
		return ctx, false, fmt.Errorf("write body buffer: %w", err)
	}
