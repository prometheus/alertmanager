...
func TestJSMRedactedURL(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			cfg: &config.JSMConfig{
				...
				Responders: []config.JSMConfigResponder{
					{
...
`,
		...
		t.Run(tc.title, func(t *testing.T) {
			...
			expectedURL, _ := url.Parse("https://jsm/apiv2/alerts")
...
func TestJSMWithUpdate(t *testing.T) {
	u, err := url.Parse("https://test-jsm-url")
	require.NoError(t, err)
	tmpl := test.CreateTmpl(t)
	ctx := context.Background()
	ctx = notify.WithGroupKey(ctx, "1")
	jsmConfigWithUpdate := config.JSMConfig{
		Message:      `{{ .CommonLabels.Message }}`,
		Description:  `{{ .CommonLabels.Description }}`,
		UpdateAlerts: true,
		APIKey:       "test-api-key",
	...
	notifierWithUpdate, err := New(&jsmConfigWithUpdate, tmpl, log.NewNopLogger())
	alert := &types.Alert{
		Alert: model.Alert{
			StartsAt: time.Now(),
			EndsAt:   time.Now().Add(time.Hour),
	...
	require.Equal(t, requests[0].URL.String(), "https://test-jsm-url/v2/alerts")
	require.NotEmpty(t, body0)

	require.Equal(t, requests[1].URL.String(), fmt.Sprintf("https://test-jsm-url/v2/alerts/%s/message?identifierType=alias", alias))
	require.Equal(t, body1, `{"message":"new message"}
`)
	require.Equal(t, requests[2].URL.String(), fmt.Sprintf("https://test-jsm-url/v2/alerts/%s/description?identifierType=alias", alias))
