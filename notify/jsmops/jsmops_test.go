...
func TestJSMOpsRetry(t *testing.T) {
	notifier, err := New(
		&JSMOpsConfig{
...
func TestJSMOpsSecretNotLeaked(t *testing.T) {
		...
		&JSMOpsConfig{
			...
			HTTPConfig: &commoncfg.HTTPClientConfig{
				BasicAuth: &commoncfg.BasicAuth{
					Password: commoncfg.Secret(secret),
				},
			},
		},
		test.CreateTmpl(t),
...
func TestJSMOps(t *testing.T) {
	u, err := url.Parse("https://jsmops.example.com/jsm/ops/api/")
	require.NoError(t, err)
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			cfg: &JSMOpsConfig{
				...
				Responders: []JSMOpsConfigResponder{
					{
...
`,
		...
		t.Run(tc.title, func(t *testing.T) {
			...
			expectedURL, _ := url.Parse("https://jsmops.example.com/jsm/ops/api/test-cloud-id/v1/alerts")
...
func TestJSMOpsWithUpdate(t *testing.T) {
	u, err := url.Parse("https://jsmops.example.com/jsm/ops/api/")
	require.NoError(t, err)
	tmpl := test.CreateTmpl(t)
	ctx := context.Background()
	...
	notifier, err := New(&cfg, tmpl, promslog.NewNopLogger())
	require.NoError(t, err)
	...
	requests, retry, err := notifier.createRequests(ctx, alert)
	require.NoError(t, err)
	require.True(t, retry)
	...
	require.Equal(t, "https://jsmops.example.com/jsm/ops/api/test-cloud-id/v1/alerts", requests[0].URL.String())
	require.NotEmpty(t, body0)

	require.Equal(t, "PATCH", requests[1].Method)
	require.Equal(t, fmt.Sprintf("https://jsmops.example.com/jsm/ops/api/test-cloud-id/v1/alerts/%s/message?identifierType=alias", alias), requests[1].URL.String())
	require.JSONEq(t, `{"message":"new message"}`, body1)
	require.Equal(t, "PATCH", requests[2].Method)
	require.Equal(t, fmt.Sprintf("https://jsmops.example.com/jsm/ops/api/test-cloud-id/v1/alerts/%s/description?identifierType=alias", alias), requests[2].URL.String())
