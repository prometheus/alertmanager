...
type Notifier struct {
	conf      *config.GooglePubsubWebhookConfig
...
// New returns a new Google Pubsub webhook receiver.
...
func New(conf *config.GooglePubsubWebhookConfig, t *template.Template, l log.Logger) (*Notifier, error) {
	...
	_ = level.Info(l).Log("msg", "Google Pubsub Webhook ServiceAccountFile", "ServiceAccountFile", serviceAccountFilePath)
	...
	_ = level.Info(l).Log("msg", "Google Pubsub Webhook URL", "url", targetUrl)
...
// ValidateConfig validates the Google Pubsub Webhook configuration.
func ValidateConfig(conf *config.GooglePubsubWebhookConfig) error {
