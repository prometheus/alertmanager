...
func (rcrt rocketchatRoundTripper) RoundTrip(req *http.Request) (res *http.Response, e error) {
	req.Header.Set("X-Auth-Token", rcrt.token)
	req.Header.Set("X-User-Id", rcrt.tokenID)
	return rcrt.wrapped.RoundTrip(req)
}
...
func (n *Notifier) Notify(ctx context.Context, as ...*types.Alert) (bool, error) {
	...
	if numActions > 0 {
		...
		for index, action := range n.conf.Actions {
			rocketchatAction := config.RocketchatAttachmentAction{
			...
			actions[index] = rocketchatAction
		}
		att.Actions = actions
	}
