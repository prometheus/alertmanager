...
// Notifier implements a Notifier for generic sigmasms.
...
func New(conf *config.SigmaConfig, t *template.Template, l log.Logger, httpOpts ...commoncfg.HTTPClientOption) (*Notifier, error) {
	...
	return &Notifier{
		...
		logger: l,
		client: client,
		// Sigmasmss are assumed to respond with 2xx response codes on a successful
...
// Message defines the JSON object send to Sigmasms endpoints.
type Message struct {
	*template.Data
