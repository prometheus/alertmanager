...
func TestResolver_URLForMethod_ValidScenarios(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "method chat.update with apiURLFile",
			apiURL:     nil,
			apiURLFile: defaultAPIURLFile.Name(),
			method:     updateChatMethod,
			want:       baseURL + "/file/" + updateChatMethod,
		},
		{
			name:       "method chat.update with apiURLFile with empty spaces",
			apiURL:     nil,
		...
		{
			name:       "method chat.update with apiURLFile with trailing slash",
			apiURL:     nil,
