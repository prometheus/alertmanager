...
func TestSlackThreadMessage(t *testing.T) {
	...
	notifier, err := New(
		&Config{
			...
			Channel:         "#alerts",
			MessageStrategy: PublishingStrategyThread,
		},
		test.CreateTmpl(t),
...
func TestSlackThreadResolveEmoji(t *testing.T) {
	...
	notifier, err := New(
		&Config{
			...
			Channel:         "#alerts",
			MessageStrategy: PublishingStrategyThread,
			ThreadedOptions: &SlackThreadedOptions{ResolveEmoji: "white_check_mark"},
		},
		test.CreateTmpl(t),
		promslog.NewNopLogger(),
	)
...
func TestSlackThreadDirectMode(t *testing.T) {
	...
	notifier, err := New(
		&Config{
			...
			Channel:         "#alerts",
			MessageStrategy: PublishingStrategyThread,
			ThreadedOptions: &SlackThreadedOptions{UseSummaryHeader: &useSummary},
		},
		test.CreateTmpl(t),
		promslog.NewNopLogger(),
	)
...
func TestSlackThreadDirectModeResolveEmoji(t *testing.T) {
	...
	notifier, err := New(
		&Config{
			...
			Channel:         "#alerts",
			MessageStrategy: PublishingStrategyThread,
			ThreadedOptions: &SlackThreadedOptions{
