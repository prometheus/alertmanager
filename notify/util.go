...
func (r *Retrier) Check(resp *http.Response) (bool, error) {
	...
	retry := resp.StatusCode/100 == 5 || slices.Contains(r.RetryCodes, resp.StatusCode)

	if resp.StatusCode == http.StatusTooManyRequests {
		retryAfter := parseRetryAfter(resp.Header)
		e := NewErrorWithReason(
			TooManyRequestsReason,
			errors.New("received 429 Too Many Requests"),
		)
		// if the server provided a Retry-After header, use that value otherwise, use the
		// default retry after duration defined in NewErrorWithReason for TooManyRequestsReason.
		if retryAfter > 0 {
