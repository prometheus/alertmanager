...
func (n *QLNotifies) Get(dest string, fps ...model.Fingerprint) ([]*types.NotifyInfo, error) {
...
func (n *QLNotifies) Set(ns ...*types.NotifyInfo) error {
	tx, err := n.db.Begin()
	if err != nil {
		return err
	}
...
func NewQLAlerts(db *sql.DB) (*QLAlerts, error) {
	tx, err := db.Begin()
	if err != nil {
		return nil, err
	}
...
func (a *QLAlerts) Subscribe() provider.AlertIterator {
	var (
...
func (a *QLAlerts) GetPending() provider.AlertIterator {
	var (
		ch   = make(chan *types.Alert, 200)
		done = make(chan struct{})
	)
...
func (a *QLAlerts) getPending() ([]*types.Alert, error) {
...
func (a *QLAlerts) Get(model.Fingerprint) (*types.Alert, error) {
	return nil, nil
}

// Put implements the Alerts interface.
func (a *QLAlerts) Put(alerts ...*types.Alert) error {
	tx, err := a.db.Begin()
	if err != nil {
		return err
	}
...
type QLSilences struct {
	db     *sql.DB
	marker types.Marker
}

// NewSQLSilences returns a new SQLSilences based on the provided SQL DB.
func NewQLSilences(db *sql.DB, mk types.Marker) (*QLSilences, error) {
	tx, err := db.Begin()
	if err != nil {
		return nil, err
	}
	...
	return &QLSilences{db: db, marker: mk}, nil
}
...
func (s *QLSilences) Mutes(lset model.LabelSet) bool {
	sils, err := s.All()
	if err != nil {
...
func (s *QLSilences) All() ([]*types.Silence, error) {
	rows, err := s.db.Query(`
...
func (s *QLSilences) Set(sil *types.Silence) (uint64, error) {
...
func (s *QLSilences) Del(sid uint64) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
...
func (s *QLSilences) Get(sid uint64) (*types.Silence, error) {
	row := s.db.QueryRow(`
		SELECT id(), matchers, starts_at, ends_at, created_at, created_by, comment
		FROM silences
		WHERE id() == $1
